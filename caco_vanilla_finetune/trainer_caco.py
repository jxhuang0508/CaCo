...
from model.deeplab_advent_caco import get_deeplab_v2
...
from utils.loss import CaContrast_loss
...
class AD_Trainer(nn.Module):
    ...
    def update_cross_entropy(self, labels, pred1):
            criterion = nn.CrossEntropyLoss(weight = self.class_weight, ignore_index=255, reduction = 'none')
    ...
    def queue_update(self, queue, PL_queue, queue_prob, f_src_main, labels):
